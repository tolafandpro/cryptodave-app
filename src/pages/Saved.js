...
const SavedBtn = ({data}) => {
    ...
    const {saveCoin, allCoins, removeCoin, resetSavedResult } = useContext(StorageContext);
            ...
            <div className="w-full min-h-[60vh] py-8 border border-gray-100 rounded">
                {
                                ...
                                savedData.map((data) => {
                                    return(
                                        <tr key={data.id} className="text-center text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0">
                                            ...
                                            <td className="py-4 md:table-cell hidden text-red">{data.market_cap_change_percentage_24h}%</td>
                    ...
                    : 
                    <h1 className='min-h-[60vh] text-lg text-Pneon-50 flex items-center justify-center'> 
                        You don't have any favorite data to display 
                    </h1>
                }
                
                <span onClick={resetSavedResult} className='hover:scale-110 transition-all transition-ease cursor-pointer absolute right-10 -top-9'>reset</span>
                <button onClick={resetSavedResult}
                    className=" flex w-[2rem] ml-4 hover:scale-110 transition-all transition-ease absolute right-0 -top-10">
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="2em"
                    height="2em"
